            ...
            for (var j in laby1[i]) {
                ...
                if (laby1[i][j] == 0) {
                   W16 = wall.create(Dx,Dy,'ws',16)
                    ...
                    // W3.body.loadPolygon('ws-phy','Mur')
                }
                if (laby1[i][j] == 1) {
...
update: function (){
  this.movePlayer();
    ...
    movePlayer: function() {
        ...
        if (this.cursor.left.isDown || direction == 8) {
            sprite.body.velocity.x = -SPEED;
            sprite.body.velocity.y = 0;
            sprite.play('left',5,true)
            direction = 8
        }
        else if (this.cursor.right.isDown || direction == 2) {
            sprite.body.velocity.x = SPEED;
            sprite.body.velocity.y = 0;
            sprite.play('right',5,true)
            direction = 2
        }
        else if (this.cursor.down.isDown || direction == 4) {
        ...
        else if(this.cursor.up.isDown || direction == 1){
            sprite.body.velocity.y = -SPEED;
            sprite.body.velocity.x = 0;
